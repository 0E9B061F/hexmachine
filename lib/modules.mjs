...
export const hrefMod =(conf)=> {
  ...
  return async function hrefMod(node) {
    node.href = node.outhref(node.filename)
  ...
  return [
    async function templateMod(node) {
      if (!conf.filename) {
        ...
        node.setFilename(conf.filename)
      }
...
export const timebubbleMod =conf=> {
  conf = {
    prop: "posts",
    ...conf,
  }
  return async function timebubbleMod(node) {
    let item, birth, edited
    for (let n = 0; n < node[conf.prop].length; n++) {
      item = node[conf.prop][n]
      if (birth === undefined) birth = item.btime
      else birth = Math.min(birth, item.btime)
      if (edited === undefined) edited = item.mtime
      else edited = Math.max(edited, item.mtime)
    }
    if (birth !== undefined) {
      node.btime = birth
      node.bdate = new Date(node.btime).toLocaleDateString("en-US").replace(/\//g, "-")

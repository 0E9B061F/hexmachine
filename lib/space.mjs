...
export class HMDPost extends Post {
  ...
  static modules = [
    async node=> {
      node.chronological = node.conf.chronological
      const raw = await readFile(node.path, "utf-8")
      node.pre = await node.root.hmd.preParse(raw)
      node.conf = { ...this.defaults, ...node.pre.data }
    },
    infoMod,
    async node=> {
      const hmd = await node.root.hmd.postParse(node.pre, node)
      node.toc = hmd.toc
      node.content = hmd.html
      if (hmd.stats.images.first) {
        node.image = hmd.stats.images.first
      }
    },
    ...this.modules,
    templateMod("post", {
...
export class HMDArticle extends HMDPost {
  static defaults = {
    chronological: false,
  }
}

export class HMDPage extends HMDPost {
  static defaults = {
    chronological: true,
  }
  static modules = [
    ...this.modules,
    async node => {
      node.parsedpg = parseInt(node.core)
      node.page = node.conf.page || node.parsedpg
    },
    templateMod("page", {
      ctxSelf: node => {
        return { toc: node.hasToc ? node.toc : null }
...
export const sortPosts =conf=>{
  conf = {
    prop: "posts",
    by: "btime",
    ...conf,
  }
  return async function sortPosts(node) {
    node[conf.prop].sort((a, b) => b[conf.by] - a[conf.by])
  }
}
...
export class Index extends Space {
  static modules = [
    ...
    templateMod("blog", {
      ctxSelf: node => {
        return { list: node.clip(), showns: true }
      },
    }),
  ]
}
